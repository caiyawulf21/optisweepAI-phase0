...
class AzureKnowledgeSettings:
    cosmos_endpoint: str | None = field(
        default_factory=lambda: os.getenv("AZURE_COSMOS_ENDPOINT")
        or os.getenv("COSMOS_ENDPOINT")
    )
    cosmos_key: str | None = field(
        default_factory=lambda: os.getenv("AZURE_COSMOS_KEY") or os.getenv("COSMOS_KEY")
    )
    cosmos_database_name: str = field(
        default_factory=lambda: os.getenv("AZURE_COSMOS_DATABASE_NAME")
        or os.getenv("COSMOS_DATABASE")
        or "optisweep_knowledge_phase0"
    )
    search_endpoint: str | None = field(
        default_factory=lambda: os.getenv("AZURE_SEARCH_ENDPOINT")
    )
    search_key: str | None = field(default_factory=lambda: os.getenv("AZURE_SEARCH_KEY"))
    ...
    raw_artifacts_container: str = field(
        default_factory=lambda: os.getenv(
            "AZURE_RAW_ARTIFACTS_CONTAINER", "raw-source-artifacts"
        )
    )
    processed_artifacts_container: str = field(
        default_factory=lambda: os.getenv(
            "AZURE_PROCESSED_ARTIFACTS_CONTAINER",
    ...
    cosmos_vector_dimensions: int = field(
        default_factory=lambda: int(
            os.getenv(
                "AZURE_COSMOS_VECTOR_DIMENSIONS",
                os.getenv("AZURE_SEARCH_VECTOR_DIMENSIONS", "1536"),
            )
        )
    )
